/**
 * Mappings précis entre noms d'ingrédients et codes CIQUAL
 * Généré depuis nutrition_mappings_export.csv
 * 264 mappings manuels pour une précision maximale
 */

export const NUTRITION_MAPPINGS = {
  // Herbes et épices
  'herbes de provence': '11060',
  '1c. à café herbes de provence': '11060',
  'basilic': '11033',
  'basilic séché': '11032',
  'aneth frais': '11093',
  'persil': '11014',
  'persil frais': '11014',
  'ciboulette': '11003',
  'coriandre': '11094',
  'coriandre fraîche': '11094',
  'coriandre en poudre': '11026',
  'coriandre moulu': '11026',
  'menthe': '11027',
  'thym': '11038',
  'thym frais': '11070',
  'origan': '11035',
  'origan séché': '11035',
  'cannelle': '11025',
  'cannelle en poudre': '11025',
  'cumin': '11042',
  'curcuma': '11089',
  'paprika': '11049',
  'safran': '11039',
  'gingembre': '11074',
  'gingembre moulu': '11006',
  'noix de muscade': '11048',
  'poivre': '11015',
  'poivre moulu': '11015',
  'poivre noir moulu': '11015',
  'piment de cayenne': '11088',
  'piment de cayenne (facultatif)': '11088',
  
  // Légumes
  'tomate': '20047',
  'tomates': '20047',
  'tomates cerises': '20172',
  'tomates concassées': '20048',
  'tomates concassées en conserve': '20048',
  'concentré de tomate': '20068',
  'concentré de tomates': '20068',
  'concentre de tomates': '20068',
  'purée de tomates': '20170',
  'puree de tomates': '20170',
  'sauce tomate': '11107',
  'concombre': '20210',
  'courgette': '20020',
  'courgettes': '20020',
  'aubergine': '20002',
  'aubergines': '20002',
  'poivron': '20088',
  'poivrons': '20088',
  'poivron rouge': '20088',
  'poivron vert': '20086',
  'mini poivrons': '20088',
  'carotte': '20009',
  'carottes': '20009',
  'oignon': '20035',
  'oignons': '20035',
  'oignon rouge': '20324',
  'échalote': '20097',
  'échalotes': '20097',
  'Échalote': '20255',
  'echalote': '20255',
  'ail': '11307',
  'd\'ail': '11307',
  'gousse d\'ail': '11000',
  'gousses d\'ail': '11307',
  'brocoli': '20006',
  'chou-fleur': '20016',
  'champignon': '20056',
  'champignons': '20056',
  'champignons de paris': '20125',
  'asperges': '20299',
  'épinard': '20336',
  'épinards': '20336',
  'haricot vert': '20030',
  'haricots verts': '20030',
  'céleri': '20023',
  'céleri branche': '20023',
  'navets': '20033',
  'potiron': '20332',
  'pomme de terre': '4002',
  'pommes de terre': '4002',
  'petits pois (frais ou surgelés)': '20326',
  
  // Salades
  'salade': '11020',
  'laitue': '11020',
  'salade romaine': '25604',
  'jeunes pousses de salade': '25604',
  
  // Légumineuses
  'lentille': '20505',
  'lentilles': '20505',
  'lentilles vertes': '20505',
  'pois chiche': '20532',
  'pois chiches': '20532',
  'haricot blanc': '20502',
  'haricots blancs': '20502',
  'haricot rouge': '20503',
  'haricots rouges': '20503',
  
  // Céréales et féculents
  'riz': '9104',
  'riz blanc': '9104',
  'riz complet': '9103',
  'riz à sushi': '25088',
  'quinoa': '9341',
  'avoine': '32140',
  'flocons d\'avoine': '32140',
  'pâte': '9811',
  'pâtes': '9811',
  'pâtes semi complètes': '9811',
  'spaghetti': '9811',
  'lasagne': '9811',
  'semoule': '9683',
  'couscous': '9683',
  'pain': '9000',
  'pain blanc': '7000',
  'pain complet': '7110',
  'pain de campagne': '7261',
  'pains burger': '7259',
  'pâte à pizza': '96778',
  'tortillas de maïs': '7813',
  'farine': '9440',
  'farine de blé': '9436',
  'farine de ble': '9436',
  'maïzena': '9545',
  
  // Fruits
  'pomme': '13039',
  'pommes': '13039',
  'banane': '13005',
  'bananes': '13005',
  'orange': '13034',
  'oranges': '13034',
  'citron': '13009',
  'citrons': '13009',
  'citron vert': '13067',
  'kiwi': '13021',
  'kiwis': '13021',
  'fraise': '13014',
  'fraises': '13014',
  'avocat': '13004',
  'avocats': '13004',
  'poire': '13037',
  'poires': '13037',
  'pêche': '13193',
  'pêches': '13193',
  'abricot': '13000',
  'abricots': '13000',
  'ananas frais': '13002',
  'raisins secs': '13046',
  
  // Fruits à coque et graines
  'amandes': '15000',
  'amandes decortiquees': '15000',
  'amandes effilees et grillees': '15000',
  'amandes en poudre': '15000',
  'poudre d\'amandes': '15000',
  'noix': '15005',
  'noix mélangées (amandes, noisettes, noix de cajou)': '15005',
  'noix de coco rapee': '15007',
  'noix de coco râpée': '15007',
  'graines (chia, lin, courge)': '15047',
  'graines de sesame': '15010',
  'graines de coriandre': '11026',
  'graines de cumin': '11042',
  
  // Viandes
  'poulet': '36004',
  'poulet entier': '36004',
  'blanc de poulet': '36018',
  'filets de poulet': '36018',
  'dinde': '36306',
  'filet de dinde': '28964',
  'boeuf': '6151',
  'bœuf': '6151',
  'viande de bœuf': '6255',
  'boeuf haché': '6255',
  'steak haché': '6255',
  'viande hachee': '6255',
  'viande hachée': '6255',
  'viande hachée de boeuf': '6255',
  'entrecôte': '6100',
  'porc': '28205',
  'viande de porc': '28205',
  'veau': '6563',
  'agneau': '21503',
  'jambon': '28919',
  'jambon blanc': '28800',
  'saucisse': '30780',
  'saucisses': '30780',
  
  // Poissons et fruits de mer
  'saumon': '25996',
  'saumon frais': '26036',
  'saumon fumé': '26037',
  'pavés de saumon': '25996',
  'thon': '26179',
  'cabillaud': '25997',
  'truite fumée': '27029',
  'filet de truite': '27006',
  'sardine': '26034',
  'sardines': '26034',
  'bar': '27030',
  'filet de bar': '26205',
  'foie de morue fumé': '26142',
  'crevette': '10007',
  'crevettes': '10007',
  
  // Produits laitiers
  'lait': '19041',
  'lait entier': '19023',
  'crème': '19040',
  'crème fraîche': '19410',
  'creme fraiche': '19410',
  'crème fraîche épaisse': '19410',
  'crème liquide': '19415',
  'beurre': '16400',
  'beurre (pour le moule)': '16400',
  'yaourt': '19599',
  'yaourt nature': '19593',
  'yaourt grec': '19860',
  'fromage blanc': '19646',
  'fromage': '12115',
  'emmental': '12115',
  'fromage râpé': '12118',
  'parmesan': '12120',
  'parmesan en copeaux': '12120',
  'parmesan râpé': '12120',
  'mozzarella': '19590',
  'mozzarella râpée': '19590',
  'feta': '12061',
  'camembert': '12006',
  'fromage de chèvre': '12800',
  'fromage cheddar': '12726',
  'fromage à tartiner': '12300',
  'lait de coco': '18041',
  
  // Œufs
  'oeuf': '22000',
  'oeufs': '22000',
  'œuf': '22000',
  'œufs': '22000',
  
  // Huiles et matières grasses
  'huile': '17000',
  'huile d\'olive': '17270',
  'huile d'olive': '17270',
  'huile de colza': '17130',
  'huile de tournesol': '17440',
  'huile de coco': '16040',
  
  // Condiments et sauces
  'moutarde': '11013',
  'moutarde de dijon': '11013',
  'vinaigre de riz': '11018',
  'vinaigre de xeres': '11220',
  'sauce soja': '11104',
  'capre au vinaigre': '11040',
  'olives vertes': '13033',
  
  // Sucres et produits sucrés
  'sucre': '31016',
  'sucre blanc': '31016',
  'sucre en poudre': '31016',
  'cassonade': '31017',
  'miel': '31008',
  'confiture': '31006',
  'chocolat noir': '31005',
  'chocolat patissier': '31005',
  'pepites de chocolat': '31005',
  'pépites de chocolat': '31005',
  
  // Bouillons et bases
  'bouillon de légumes': '25948',
  'bouillon de volaille': '25947',
  'bouillon de boeuf': '25930',
  'eau': '18000',
  
  // Divers
  'sel': '11017',
  'gros sel': '11096',
  'levure chimique': '11046',
  'levure boulangere': '11045',
  'chapelure': '7500',
  'feuilles de nori': '20987',
  'compote de pommes sans sucre': '13109',
  'jus de citron': '2007',
  'jus d\'orange': '2013',
  'vin rouge': '5214'
};

/**
 * Normalise le nom d'un ingrédient pour le mapping
 * @param {string} ingredientName - Nom de l'ingrédient
 * @returns {string} - Nom normalisé
 */
export const normalizeIngredientName = (ingredientName) => {
  if (!ingredientName) return '';
  
  return ingredientName
    .toLowerCase()
    .trim()
    // Enlever les articles
    .replace(/^(le|la|les|l'|un|une|des|du|de la|de l')\s+/i, '')
    // Enlever les quantités
    .replace(/^\d+\s*(g|kg|ml|cl|l|c\.|c\.à|cuillère|cuillères|pincée|gousse|gousses|tranche|tranches)?\s*/i, '')
    // Enlever les parenthèses et leur contenu
    .replace(/\s*\([^)]*\)/g, '')
    // Enlever les indications finales
    .replace(/\s*(cru|crue|cuit|cuite|frais|fraîche|séché|séchée|râpé|râpée|haché|hachée|en poudre|en dés)$/i, '')
    .trim();
};

/**
 * Trouve le code CIQUAL pour un ingrédient
 * @param {string} ingredientName - Nom de l'ingrédient
 * @returns {string|null} - Code CIQUAL ou null si non trouvé
 */
export const findCiqualCodeFromMapping = (ingredientName) => {
  if (!ingredientName) return null;
  
  // Normaliser le nom
  const normalized = normalizeIngredientName(ingredientName);
  
  // Recherche exacte
  if (NUTRITION_MAPPINGS[normalized]) {
    console.log(`✅ [Mapping] Trouvé exact: "${ingredientName}" → code ${NUTRITION_MAPPINGS[normalized]}`);
    return NUTRITION_MAPPINGS[normalized];
  }
  
  // Recherche avec le nom original
  const originalLower = ingredientName.toLowerCase().trim();
  if (NUTRITION_MAPPINGS[originalLower]) {
    console.log(`✅ [Mapping] Trouvé original: "${ingredientName}" → code ${NUTRITION_MAPPINGS[originalLower]}`);
    return NUTRITION_MAPPINGS[originalLower];
  }
  
  // Recherche partielle (si l'ingrédient contient un mot-clé)
  for (const [key, code] of Object.entries(NUTRITION_MAPPINGS)) {
    if (normalized.includes(key) || key.includes(normalized)) {
      console.log(`✅ [Mapping] Trouvé partiel: "${ingredientName}" ≈ "${key}" → code ${code}`);
      return code;
    }
  }
  
  console.log(`⚠️ [Mapping] Non trouvé: "${ingredientName}" (normalisé: "${normalized}")`);
  return null;
};

/**
 * Obtient tous les codes CIQUAL disponibles dans les mappings
 * @returns {Array<string>} - Liste des codes CIQUAL
 */
export const getAllMappedCiqualCodes = () => {
  return [...new Set(Object.values(NUTRITION_MAPPINGS))];
};

/**
 * Statistiques sur les mappings
 */
export const getMappingStats = () => {
  const uniqueCodes = getAllMappedCiqualCodes();
  return {
    totalMappings: Object.keys(NUTRITION_MAPPINGS).length,
    uniqueCiqualCodes: uniqueCodes.length,
    mappingsPerCode: (Object.keys(NUTRITION_MAPPINGS).length / uniqueCodes.length).toFixed(2)
  };
};

console.log('📚 [Mappings] Chargé:', getMappingStats());
